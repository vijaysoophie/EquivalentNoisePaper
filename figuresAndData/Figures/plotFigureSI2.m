...
% figure; 
% hold on;
% h = boxplot([AllThresholdsExperiment2 AllThresholdsExperiment3].^2, 'OutlierSize',8);
% set(h,'LineWidth',3);
% set(gca,'LineWidth',1);
% xticks([1:6]);
% xticklabels({'1','2','2a', '3', '3a', '\sigma = 1'});
% box on;
% set(gca, 'FontSize', 20, 'yscale', 'log');
% xlabel('Condition number');
% ylabel('T^2');
...
% hold on;
% h = boxplot(log10([mean(ThresholdSubject4) mean(ThresholdExperiment3_observer2);
%              mean(ThresholdSubject5) mean(ThresholdExperiment3_observer4);
%              mean(ThresholdSubject8) mean(ThresholdExperiment3_observer8);
%              mean(ThresholdSubject11) mean(ThresholdExperiment3_observer17);].^2), 'OutlierSize',8);
% set(h,'LineWidth',3);
% set(gca,'LineWidth',1);
% xticks([1:6]);
% xticklabels({'1','2','2a', '3', '3a', '$\sigma = 1$'});
