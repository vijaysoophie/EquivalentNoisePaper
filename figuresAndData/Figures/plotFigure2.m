...
LRVLevels = data(3:end,1);
proportionCorrect = data(3:end,2);
totalTrial  = data(2,2)*ones(size(proportionCorrect));

%% Plot Figure

hFig = figure();
yLimits = [-0.05 1.05];
xLimits = [(min(LRVLevels) - min(diff(LRVLevels))/2) ...
    (max(LRVLevels)+ min(diff(LRVLevels))/2)];
set(hFig,'units','pixels', 'Position', [1 1 600 500]);
hold on; box on;

%% plot a vertical line indicating the standard
lStdY = plot([LRVLevels(6) LRVLevels(6)], yLimits,':r','LineWidth', 2);
...
paramsValues0 = [mean(LRVLevels) 1/((max(LRVLevels)-min(LRVLevels))) 0 0];
...
[paramsValues] = PAL_PFML_Fit(...
    LRVLevels, proportionCorrect, totalTrial, ...
...
lData = plot(LRVLevels,proportionCorrect./totalTrial,'r.','MarkerSize',40);
%%
...
legend([lData lTh lStdY],...
    {'Observation', 'Cumulative Gaussian', 'Standard LRV'},...
